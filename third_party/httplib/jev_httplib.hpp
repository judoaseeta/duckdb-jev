...
#ifndef JEV_CPPHTTPLIB_HTTPLIB_H
#define JEV_CPPHTTPLIB_HTTPLIB_H
...
inline ssize_t write_response_line(Stream &strm, int status) {
  ...
  s += " ";
  s += duckdb_jev_httplib::status_message(status);
  s += "\r\n";
  return strm.write(s.data(), s.size());
}
...
#endif // JEV_CPPHTTPLIB_HTTPLIB_H
